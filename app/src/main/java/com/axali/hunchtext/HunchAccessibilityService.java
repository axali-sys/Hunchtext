...
public class HunchAccessibilityService extends AccessibilityService {
    ...
    @Override public void onServiceConnected() {
        ...
        if (info != null) {
            info.eventTypes = AccessibilityEvent.TYPE_VIEW_FOCUSED | AccessibilityEvent.TYPE_VIEW_TEXT_CHANGED | AccessibilityEvent.TYPE_WINDOWS_CHANGED;
    ...
    private void showHunchButton() {
        ...
        overlay = b;
        overlayParams = new WindowManager.LayoutParams(54, 54, WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY, WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE, PixelFormat.TRANSLUCENT);
    ...
    private void positionNearKeyboard() {
        ...
        for (AccessibilityWindowInfo w : getWindows()) if (w.getType() == AccessibilityWindowInfo.TYPE_INPUT_METHOD) { ime = w; break; }
        if (ime == null) return;
        Rect r = new Rect(); ime.getBoundsInScreen(r);
        overlayParams.gravity = Gravity.TOP | Gravity.RIGHT;
        overlayParams.x = 68;
    ...
    private void showHunchChoices() {
        ...
        LinearLayout panel = new LinearLayout(this); panel.setOrientation(LinearLayout.HORIZONTAL);
        for (String choice : choices) {
            Button b = new Button(this); b.setText(choice); b.setAllCaps(false);
        ...
        overlay = panel;
        overlayParams = new WindowManager.LayoutParams(WindowManager.LayoutParams.MATCH_PARENT, 70, WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY, WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE, PixelFormat.TRANSLUCENT);
        overlayParams.gravity = Gravity.TOP; overlayParams.y = findKeyboardTop();
        wm.addView(panel, overlayParams);
    }

    private int findKeyboardTop() {
        for (AccessibilityWindowInfo w : getWindows()) if (w.getType() == AccessibilityWindowInfo.TYPE_INPUT_METHOD) { Rect r = new Rect(); w.getBoundsInScreen(r); return Math.max(0, r.top + 4); }
        return 900;
    }
    ...
    private void applyHunch(String text, String mode) {
        ...
        if (n != null) { Bundle args = new Bundle(); args.putCharSequence(AccessibilityNodeInfo.ACTION_ARGUMENT_SET_TEXT_CHARSEQUENCE, out); n.performAction(AccessibilityNodeInfo.ACTION_SET_TEXT, args); }
