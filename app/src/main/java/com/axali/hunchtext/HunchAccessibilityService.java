...
public class HunchAccessibilityService extends AccessibilityService {
    ...
    private void showHunchChoices(){String text=focusedNode==null||focusedNode.getText()==null?"":focusedNode.getText().toString().trim();if(text.isEmpty()){Toast.makeText(this,"Type something first, then tap HunchText.",Toast.LENGTH_SHORT).show();return;}final String[] choices={"❤️ Warm","😊 Friendly","🌿 Gentle","💬 Direct","🔥 Confident"};LinearLayout panel=new LinearLayout(this);panel.setOrientation(LinearLayout.HORIZONTAL);for(String choice:choices){Button b=new Button(this);b.setText(choice);b.setAllCaps(false);b.setOnClickListener(v->applyHunch(text,((Button)v).getText().toString()));panel.addView(b,new LinearLayout.LayoutParams(0,62,1));}if(overlay!=null)wm.removeView(overlay);overlay=panel;overlayParams=new WindowManager.LayoutParams(WindowManager.LayoutParams.MATCH_PARENT,70,WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY,WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE,PixelFormat.TRANSLUCENT);overlayParams.gravity=Gravity.TOP;overlayParams.y=findKeyboardTop();wm.addView(panel,overlayParams);}
    private int findKeyboardTop(){for(AccessibilityWindowInfo w:getWindows())if(w.getType()==AccessibilityWindowInfo.TYPE_INPUT_METHOD){Rect r=new Rect();w.getBoundsInScreen(r);return Math.max(0,r.top+4);}return 900;}
    private void applyHunch(String text,String mode){String out=text;if(mode.contains("Warm"))out="I really mean this: "+text+" ❤️";else if(mode.contains("Friendly"))out="Just wanted to say: "+text+" 😊";else if(mode.contains("Gentle"))out="I just wanted to share that "+text+" 🌿";else if(mode.contains("Confident"))out=text+" — I mean it. 🔥";AccessibilityNodeInfo n=focusedNode;if(n!=null){Bundle args=new Bundle();args.putCharSequence(AccessibilityNodeInfo.ACTION_ARGUMENT_SET_TEXT_CHARSEQUENCE,out);n.performAction(AccessibilityNodeInfo.ACTION_SET_TEXT,args);}else Toast.makeText(this,out,Toast.LENGTH_LONG).show();if(overlay!=null){wm.removeView(overlay);overlay=null;overlayParams=null;showHunchButton();positionNearKeyboard();}}
    @Override public void onInterrupt(){} @Override public void onDestroy(){if(overlay!=null&&wm!=null)wm.removeView(overlay);overlay=null;super.onDestroy();}
}
